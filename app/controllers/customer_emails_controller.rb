...
=end
    ...
    if session[:customer].email_allowed == false
      @error = true
      @error_message = "Vous n'êtes pas autorisé à émettre des emails. Veuillez contacter l'administrateur."
    end

    if session[:customer].bulk_email.to_i < 1
      @error = true
      @error_message = "Vous n'avez plus d'emails'. Votre message n'a pas pu être envoyé."
    end
    ...
    if @error_status == 0
      ...
      redirect_to customer_finalize_email_profile_path(profile_id: @profile.id)
    end
  end
          ...
          #Thread.new do
            ...
            profile_data.each do |pd|
              ...
              if not_an_email?(email)
                ...
                send_message_request(email, @object, @sender, @message)
                @transaction.email_logs.create(subscriber_id: (session[:customer].id rescue nil), email: email, profile_id: @profile.id, message: @message, subject: @object, sender: @sender) rescue nil
                @message = @message_backup
                sent_messages += 1
              end
            end
            @transaction.update_attributes(ended_at: DateTime.now, send_messages: sent_messages, user_id: (session[:customer].id rescue nil))
            session[:customer].update_attributes(bulk_email: (@service.bulk_email.to_i rescue nil) - sent_messages) rescue nil
          #end
    ...
    Thread.new do
      ...
      @spreadsheet.each do |row|
        ...
        unless not_an_email?(email)
          if session[:customer].bulk_email.to_i > 0
            send_message_request(email, @object, @sender, @message)
          end
        end
      end
  ...
  def set_transaction(description, subscribers_count)
    @transaction = EmailTransaction.create(started_at: DateTime.now, profile_id: @profile.id, description: description, number_of_messages: subscribers_count, customer_id: session[:customer].id)
