class MessagesController < ApplicationController
  ...
  def filter_api_send_message
    ...
    @service = Customer.where("login = ? AND password = pgp_sym_decrypt(?, 'Pilote2017@key#') AND service_id = ?", @login, @password[14, @password.length], @service_id).first.id rescue nil
