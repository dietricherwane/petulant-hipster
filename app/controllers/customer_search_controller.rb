class CustomerSearchController < ApplicationController
  ...
  def set_query_params
    ...
    if !@msisdn.blank?
      ...
      @message_logs = MessageLog.where(message_logs_query + " AND customer_id = #{session[:customer].id}")
      ...
      @transaction_logs = SmsTransaction.where(transaction_logs_query + "#{transaction_logs_query.blank? ? '' : ' AND'}" + " customer_id = #{session[:customer].id}")
